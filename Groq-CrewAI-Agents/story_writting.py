...
def generateimage(chapter_content_and_character_details: str) -> str:
    ...
    filepath = os.path.join(os.getcwd() + r"\Groq-CrewAI-Agents\output", filename)
    print(filepath)
...
def writestory():
    ...
    model_name="llama3-8b-8192"#  gpt-4-turbo-preview or mixtral-8x7b-32768 ,
    ,temperature=0
    )
    ...
    task_outline = Task(
        description='Create an outline for the children\'s storybook about Animals, detailing chapter titles and character descriptions for 2 chapters.',
        agent=story_outliner,
        expected_output='A structured outline document containing 2 chapter titles, with detailed character descriptions and the main plot points for each chapter.'
    )
    ...
    task_write = Task(
        ...
        agent=story_writer,
        expected_output='A complete manuscript of the children\'s storybook about Animals with 2 chapters. Each chapter should contain approximately 100 words, following the provided outline and integrating the characters and plot points into a cohesive narrative.'
    )

    task_image_generate = Task(
        description='Generate 2 images that captures the essence of the children\'s storybook about Animals, aligning with the themes, characters, and narrative outlined for the chapters. Do it one by one.',
